...
$(document).ready(function() {
  ...
  loadData();
  setTimeout(function(){ loadSnippetDict(); }, 500);
});
...
function bindD3() {
  ...
  d3.selectAll('.articles-container').data(data[year_month].articles).enter();

  var word_list_items = d3.selectAll('td.list-count')
                          .data(data[year_month].words).enter()
            d3.selectAll('td.list-count').style('background', function(d){
                        return colorWords(d.count);
                      });

  d3.selectAll('.articles-container').select('td.td-title').html(function(d){
    if (d['h'].length > 6) {
...
function addEventHandlers() {
  ...
  $doc.on('click', '.toggle-component', function(){
    ...
    $thisSpan = $this.parent().next('.stat-item').find('.data')
  ...
  $doc.on('click', '.toggle-graph', function(){
    ...
    if ( $this.index() === 0 ) {
      d3.select('.hussein_area').transition().duration(800).attr('opacity',0)
    }
    else {
      d3.select('.hussein_area').transition().duration(800).attr('opacity',1)
    }
  ...
  $doc.on('click', 'tr.articles-row', function(){
    ...
    if ($this.find('i').hasClass('fa-angle-down')){
      ...
      $this.closest('.articles-row').next().show();
      $this.closest('.articles-row').next().find('.toggle-text').html(snippet_modified)
      $this.closest('.articles-row').next().next().show();
      $this.closest('.articles-row').next().next().find('.toggle-nyt-link').html("<a href=" + url + " target='_blank'><i class='fa fa-angle-double-right'>&nbsp</i>Click here to read full article (" + word_count + " words)</a>")

    }

    else { // .fa-close is present

      $this.find('.fa').removeClass('fa-close hi-lite');
      $this.find('.fa').addClass('fa-angle-down');

      $this.closest('.articles-row').removeClass('selected-articles-row');
      $this.closest('.articles-row').next().find('.toggle-text').empty();
      $this.closest('.articles-row').next().hide();

      $this.closest('.articles-row').next().next().find('.toggle-nyt-link').empty();
      $this.closest('.articles-row').next().next().hide();
...
function updateStatItem($thisSpan, $thisHTML, $thisIndex) {
  ...
  $thisSpan.fadeOut('fast', function(){
    ...
    if ($thisHTML === 'this month') {
      if ($thisIndex === 0){ $thisSpan.html( data[year_month].articles.length) }
      else if ($thisIndex === 1){ $thisSpan.html( data[year_month].hussein_count) }
      else { $thisSpan.html( data[year_month].overlap_counter) }
    }
    else {
      if ($thisIndex === 0){ $thisSpan.html( formatK(constants[$thisIndex])) }
      else if ($thisIndex === 1){ $thisSpan.html( formatK(constants[$thisIndex])) }
      else { $thisSpan.html( formatK(constants[$thisIndex])) }
    }
...
function clickDirection(direction) {
  ...
  if (direction == 'left') {
    ...
    if (newIndex > -1) {
      ...
      d3.select("#output").style('opacity', 0)
...
function makeChart() {
  ...
  y = d3.scale.linear().range([height,0]).domain([0, 669]);
  ...
  area = d3.svg.area()
        ...
        .y0(height)
        .y1(function(d) {return y( d.articles ); });

  lineGen = d3.svg.line()
        .x(function( d ) {return x( new Date(d.date) ); })
        .y(function( d ) {return y( d.articles ); });

  lineGen_hussein = d3.svg.line()
        .x(function( d ) {return x( new Date(d.date) ); })
        .y(function( d ) {return y( d.articles ); });
  ...
  svg.append('svg:path')
   ...
   svg.append('path')
    ...
    var handleConnector = slider.append("svg:line")
    ...
    var handleBaseline = slider.append("circle")
    ...
    var handleTop = slider.append("circle")
    ...
    var death_text = svg.append('text')
    ...
    var article_span = slider.append("svg:line")
        ...
        .attr("opacity", 0)
        .style("stroke", "#1c1c1c");
    var span_left = svg.append('circle')
        .attr('cx', x(new Date('2001-09')))
        .attr('cy', 2)
        .attr("r", 4)
        .attr('opacity', 0)
        ...
        .attr("stroke", "#1c1c1c")
        .attr("fill", "#f7f7f7")
    var span_right = svg.append('circle')
        .attr('cx', x(new Date('2011-05')))
        .attr('cy', 2)
        .attr("r", 4)
        .attr('opacity', 0)
    ...
    nine11.transition().duration(2000).attr('r', 30).attr('opacity', 0.85).transition().duration(2000).attr('r', 1).attr('opacity', 0)
    nine11_text.transition().delay(500).duration(1500).attr('opacity', 1).transition().duration(1500).attr('opacity', 0)

    setTimeout(function(){

        death.transition().duration(2000).attr('r', 30).attr('opacity', 0.85).transition().duration(2000).attr('r', 1).attr('opacity', 0)
        death_text.transition().delay(500).duration(1500).attr('opacity', 1).transition().duration(1500).attr('opacity', 0)

    }, 2500)

    setTimeout(function(){

        article_span.transition().duration(2000).attr('opacity', 1).transition().duration(1200).attr('opacity', 0)
        span_left.transition().duration(1500).attr('opacity', 1).transition().duration(2500).attr('opacity', 0)
        span_right.transition().duration(1500).attr('opacity', 1).transition().duration(2500).attr('opacity', 0)
        span_text.transition().duration(2000).attr('opacity', 1).transition().duration(1800).attr('opacity', 0)
    ...
    setTimeout(function(){
      ...
      $('#template_holder').css('visibility','visible').hide().fadeIn(1200);

      animateNumbers();

      $('#date').fadeIn(1200);
      d3.selectAll('.handle').transition().duration(2000).style('opacity', 1);
      $('footer').show();
...
function animateNumbers() {
  ...
  $('#daily-article-avg').prop('number', dailyAvgBefore).animateNumber({ number: dailyAvg, numberStep: comma_separator_number_step }, 350)
  ...
  $('#monthly-word-avg').prop('number', monthyWordBefore).animateNumber({ number: monthlyWordAvg, numberStep: comma_separator_number_step }, 350)

  var thisMonth = data[year_month].articles.length;
  $('#this-month').prop('number', prevMonth).animateNumber({ number: thisMonth, numberStep: comma_separator_number_step }, 350)
...
function animateMarker() {
  ...
  d3.select('.handle-bottom').transition().duration(350).attr("cx", x(new Date(year_month)));
...
function resize() {
  ...
  d3.select('path.hussein_line')
    .attr('d', lineGen_hussein(hussein_line_data))
